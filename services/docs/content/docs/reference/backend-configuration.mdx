...
| `scanner.xray_concurrency`                                  | `2`                       | Slots per Xray preparation/import pool; restart required                    |
| `scanner.xray_max_active`                                   | `32`                      | Xray coordinators including remote waits; restart required                  |
| `scanner.xray_warmup_timeout_seconds`                       | `600`                     | Image preparation and warm-up deadline including retries                    |
| `scanner.xray_provider_timeout_seconds`                     | `900`                     | Deadline for each provider-status or summary wait                           |
| `scanner.xray_timeout_seconds`                              | `3600`                    | Overall Xray scan deadline after worker claim                               |
...
The scanner-settings API also accepts positive integer fields `xray_concurrency`, `xray_max_active`, `xray_warmup_timeout_seconds`, `xray_provider_timeout_seconds`, and `xray_timeout_seconds`. Xray capacity changes appear in `restart_required`; timeout settings apply to future stages/scans. Environment overrides use, for example, `BACKEND_SCANNER_XRAY_WARMUP_TIMEOUT_SECONDS`. See [Xray capacity and upgrade notes](/operate/scanners-and-xray#xray-capacity-and-deadlines), including migration 116 and the separation of heartbeat from progress timestamps.
