...
The chart exposes Xray scheduling and deadline settings under `backend.config.scanner`: `xrayConcurrency` (default `2` slots in each preparation/import pool), `xrayMaxActive` (default `32` active Xray coordinators), `xrayWarmupTimeoutSeconds` (default `600`), `xrayProviderTimeoutSeconds` (default `900`), and `xrayTimeoutSeconds` (default `3600`). Capacity changes take effect on the next backend rollout; deadline changes apply to newly started Xray stages.
