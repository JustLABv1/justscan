...
func GetUserDetails(context *gin.Context, db *bun.DB) {
	...
	err = db.NewSelect().Model(&user).Column("id", "username", "email", "role", "disabled", "disabled_reason", "auth_type", "last_login_at", "last_login_method", "workspace_tour_completed_at", "created_at", "updated_at").Where("id = ?", userID).Scan(context)
	if err != nil {
