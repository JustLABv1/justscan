...
Add a registry in **Workspace setup → Registries**, select the scanner provider, and test the connection. For an image that has not been pushed, use the CLI local-image or archive workflow instead. See [registries and Xray](/scan-and-analyze/registries-and-xray) and [scan sources](/scan-and-analyze/scan-sources).
