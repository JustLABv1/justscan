...
import type { GitDiscoveredImage, GitRepository, GitRepositoryDiscoveryRule, GitRepositoryRun, GitRepositoryRunCandidate, GitRepositoryRunImage } from './types/git-repositories';

export type GitRepositoryInput = Partial<GitRepository> & { credential?: string; org_id?: string };

export const listGitRepositories = () => req<{ data: GitRepository[] }>('GET', '/api/v1/git-repositories/').then((result) => result.data ?? []);
...
export const runGitRepository = (id: string, policy?: 'changed' | 'all') => req<GitRepositoryRun>('POST', `/api/v1/git-repositories/${id}/runs`, policy ? { policy } : {});
