...
func TestPipelineTokenRouteAllowed(t *testing.T) {
	tests := []struct {
		...
		{"create archive upload session", http.MethodPost, "/api/v1/orgs/:id/archive-upload-sessions", true},
		{"upload archive chunk", http.MethodPatch, "/api/v1/orgs/:id/archive-upload-sessions/:uploadId", true},
		{"complete archive upload", http.MethodPost, "/api/v1/orgs/:id/archive-upload-sessions/:uploadId/complete", true},
