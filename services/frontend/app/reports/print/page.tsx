...
interface Scan {
  ...
  blocked_policy_details?: {
    ...
    matched_watches?: Array<{ name: string }>;
...
function filterVulns(
  vulns: Vulnerability[],
  f: Filters,
  policyFailures: Map<string, string[]>
): Vulnerability[] {
  ...
  return vulns.filter((v) => {
    ...
    if (
      search &&
      ![v.vuln_id, v.pkg_name, v.title, v.data_source].some((value) =>
...
function selectRepresentativeFinding(
  ...
  if (candidate.cvss_score !== current.cvss_score)
    return candidate.cvss_score > current.cvss_score ? candidate : current;
...
function buildReportFindings(data: ScanData[], filters: Filters): ReportFinding[] {
  ...
  for (const source of sourceFindings) {
    ...
    const key = normalizedId.startsWith('CVE-')
      ? normalizedId
      : `finding:${source.vulnerability.id}`;
    ...
    const representative = selectRepresentativeFinding(
      existing.vulnerability,
      source.vulnerability
    );
    const comments = [
      ...(existing.vulnerability.comments ?? []),
      ...(source.vulnerability.comments ?? []),
    ];
    existing.vulnerability = {
      ...representative,
      references: uniqueStrings([
        ...(existing.vulnerability.references ?? []),
        ...(source.vulnerability.references ?? []),
      ]),
      comments: comments.filter(
        (comment, index) => comments.findIndex((candidate) => candidate.id === comment.id) === index
      ),
    };
    if (
      !existing.packages.some(
        (item) =>
          item.name === packageEntry.name &&
          item.installedVersion === packageEntry.installedVersion &&
          item.fixedVersion === packageEntry.fixedVersion
      )
    ) {
      existing.packages.push(packageEntry);
    }
    existing.affectedImages = uniqueStrings([...existing.affectedImages, source.image]);
    existing.xrayPolicyLabels = uniqueStrings([
      ...existing.xrayPolicyLabels,
      ...source.xrayPolicyLabels,
    ]);
    existing.orgPolicyLabels = uniqueStrings([
      ...existing.orgPolicyLabels,
      ...source.orgPolicyLabels,
    ]);
  }
...
async function fetchScan(scanId: string, options?: ApiRequestOptions): Promise<Scan> {
  return req<Scan>('GET', `/api/v1/scans/${scanId}`, undefined, options);
}
...
async function fetchAllVulnerabilities(
  ...
  return fetchPaginatedCollection<Vulnerability>(async (page) => {
    return req<PaginatedResponse<Vulnerability>>(
      'GET',
      `/api/v1/scans/${scanId}/vulnerabilities?page=${page}&limit=${VULN_PAGE_SIZE}`,
      undefined,
      options
    );
  }, VULN_PAGE_SIZE);
...
async function fetchCompliance(
  ...
  const body = await req<{ data?: ComplianceResult[] }>(
    'GET',
...
async function fetchAllChartScans(helmChart: string, options?: ApiRequestOptions): Promise<Scan[]> {
  return fetchPaginatedCollection<Scan>(async (page) => {
    ...
    return req<PaginatedResponse<Scan>>(
      'GET',
...
async function fetchRunScans(helmRun: string, options?: ApiRequestOptions): Promise<Scan[]> {
  const detail = await req<HelmRunDetailResponse>(
    'GET',
    `/api/v1/helm/runs/${helmRun}`,
    undefined,
    options
  );
...
function ReportFilterControls({
  ...
  function toggle(sev: string) {
    const sevs = f.severities.includes(sev)
  ...
  return (
    <div className="text-sm text-zinc-900" style={{ colorScheme: 'light' }}>
      {showHeading && (
        <>
          ...
          <p className="mb-4 text-xs text-zinc-500">
            Changes apply immediately to the printable report.
          </p>
        </>
      )}
      ...
      <SearchField
        className="mb-3"
        value={f.search}
        onChange={(search) => onChange({ ...f, search })}
      >
      ...
      <NumberField
        ...
        step={0.1}
        value={f.minCvss}
        onChange={(minCvss) => onChange({ ...f, minCvss })}
      >
      ...
      {SEVS.map((s) => (
        <label
          key={s}
          className="mb-1 flex cursor-pointer items-center gap-2 text-xs text-zinc-800"
        >
          <input
            className="accent-blue-600"
            type="checkbox"
            checked={f.severities.length === 0 || f.severities.includes(s)}
            onChange={() => {
              if (f.severities.length === 0)
                onChange({ ...f, severities: SEVS.filter((x) => x !== s) });
              else toggle(s);
            }}
          />
          <span
            style={{
              ...
              height: 8,
              borderRadius: '50%',
              background: SEVERITY_DOT_COLORS[s],
  ...
  return (
    <>
      ...
      <div className="report-filter-trigger print:hidden">
        <Button className="shadow-lg" variant="primary" onPress={drawer.open}>
          Filters
        </Button>
      </div>
      ...
      >
        <Drawer.Content placement="right">
          <Drawer.Dialog
            ...
            className="flex h-full w-[min(92vw,22rem)] flex-col bg-white"
          >
            <Drawer.Header className="flex items-center justify-between border-b border-zinc-200 px-5 py-4">
              <div>
                <Drawer.Heading className="font-semibold text-zinc-900">
                  Report filters
                </Drawer.Heading>
...
function ReportVulnerabilitySection({
  findings,
  filters,
}: {
  ...
  return (
    <section style={{ marginBottom: 32 }}>
      ...
      {findings.length === 0 ? (
        <p style={{ fontSize: 13, color: '#6b7280', fontStyle: 'italic', margin: '8px 0 20px' }}>
          No vulnerabilities match current filters.
        </p>
      ) : (
        <div className="report-table-scroll">
          <table
            className="report-findings-table"
            style={{
              width: '100%',
                ...
                return (
                  <Fragment key={vulnerability.id}>
                    <tr style={{ background: rowBackground }}>
                      ...
                      >
                        {finding.packages.map((item) => (
                          <div key={`${item.name}-${item.installedVersion}-${item.fixedVersion}`}>
                            {item.name || '-'}
                          </div>
                        ))}
                      </td>
                      ...
                      >
                        {finding.packages.map((item) => (
                          <div key={`${item.name}-${item.installedVersion}`}>
                      ...
                      <td
                        style={{
                          ...
                          fontSize: 12,
                          color: '#15803d',
                          fontWeight: finding.packages.some((item) => item.fixedVersion)
                            ? 600
                            : 400,
                          overflowWrap: 'anywhere',
                        }}
                      >
                        {finding.packages.map((item) => (
                          <div key={`${item.name}-${item.fixedVersion}`}>
                            {item.fixedVersion || '-'}
                          </div>
                        ))}
                      </td>
                      <td style={{ padding: '6px 10px', border: '1px solid #e5e7eb' }}>
                        <SevBadge s={vulnerability.severity} />
                      </td>
                      <td
                        style={{
                          padding: '6px 10px',
                          ...
                          fontSize: 12,
                          textAlign: 'right',
                          fontWeight: vulnerability.cvss_score >= 7 ? 700 : 400,
                          color:
                    ...
                    {(filters.showDescription ||
                      filters.showReferences ||
                      filters.showComments ||
                      filters.showPolicyDetails ||
                      finding.affectedImages.length > 0) && (
                      <tr style={{ background: detailBackground }}>
                        <td colSpan={6} className="report-detail-cell">
                          <div style={{ marginBottom: 5 }}>
                            <strong style={{ color: '#111827' }}>Affected images:</strong>{' '}
                            {finding.affectedImages.join(' · ')}
                          </div>
                          {filters.showDescription && vulnerability.title && (
                            <strong style={{ color: '#111827' }}>{vulnerability.title} - </strong>
                          )}
                          {filters.showDescription &&
                            vulnerability.description &&
                            (vulnerability.description.length > 400
                              ? vulnerability.description.slice(0, 400) + '…'
                              : vulnerability.description)}
                          {filters.showReferences && vulnerability.references?.length > 0 && (
                            ...
                            >
                              {vulnerability.references.slice(0, 2).join(' · ')}
                            </span>
                          )}
                          {filters.showPolicyDetails &&
                            (finding.xrayPolicyLabels.length > 0 ||
                              finding.orgPolicyLabels.length > 0) && (
                              <div
                                style={{ marginTop: 5, display: 'flex', flexWrap: 'wrap', gap: 4 }}
                              >
                                {finding.xrayPolicyLabels.map((label) => (
                                  <span
                                    key={`xray-${label}`}
                                    style={{
                                      padding: '2px 5px',
                                ...
                                {finding.orgPolicyLabels.map((label) => (
                                  <span
                                    key={`org-${label}`}
                                    style={{
                                      padding: '2px 5px',
                          ...
                          {filters.showComments &&
                            ...
                            vulnerability.comments.length > 0 && (
...
function ScanSection({
  ...
  const [newFinding, setNewFinding] = useState<Partial<ManualFinding>>({
    severity: 'HIGH',
    cvss_score: 0,
  });
  ...
  const retryManualFindings = useCallback(() => {
    setManualError('');
  ...
  useEffect(() => {
    ...
    req<ManualFinding[]>('GET', `/api/v1/scans/${scan.id}/manual-findings`, undefined, {
      signal: controller.signal,
    })
      .then((data) => {
        if (!controller.signal.aborted) {
          setManualFindings(Array.isArray(data) ? data : []);
        }
      })
      .catch((error: unknown) => {
        if (!controller.signal.aborted && !isAbortError(error)) {
          setManualError(
            error instanceof Error ? error.message : 'Failed to load manual findings.'
          );
        }
      })
      .finally(() => {
  ...
  }, [manualRetryNonce, scan.id]);

  const patchImageLocation = useCallback(
    async (value: string) => {
      if (savingLocation) return;
      setSavingLocation(true);
      setLocationError('');
      try {
        await req<Scan>('PATCH', `/api/v1/scans/${scan.id}`, { image_location: value });
      } catch (error: unknown) {
        setLocationError(error instanceof Error ? error.message : 'Failed to save image location.');
      } finally {
        setSavingLocation(false);
      }
    },
    [savingLocation, scan.id]
  );

  async function submitManualFinding() {
    if (!newFinding.vuln_id || manualSaving) return;
    setManualSaving(true);
    setManualError('');
    try {
      const created = await req<ManualFinding>(
        'POST',
        `/api/v1/scans/${scan.id}/manual-findings`,
        newFinding
      );
      ...
      setManualError(error instanceof Error ? error.message : 'Failed to save manual finding.');
    } finally {
  ...
  async function deleteManualFinding(findingId: string) {
    ...
    setManualError('');
    try {
      await req<void>('DELETE', `/api/v1/scans/${scan.id}/manual-findings/${findingId}`);
      setManualFindings((prev) => prev.filter((f) => f.id !== findingId));
    } catch (error: unknown) {
      setManualError(error instanceof Error ? error.message : 'Failed to delete manual finding.');
    } finally {
    ...
    >
        ...
        >
          <span
            style={{
              ...
              border: `1px solid ${statusColors.border ?? statusColors.background}`,
            }}
          >
          ...
          {scan.architecture && (
            <span style={{ fontSize: 12, color: '#6b7280' }}>
              {scan.architecture} · {scan.os_family}
            </span>
          )}
        </div>
      </div>
        ...
        >
          <tbody>
            {(
              [
                ...(filters.showScanId
                  ? [['Scan ID', scan.id, true] as [string, string, boolean]]
                  : []),
                ...(filters.showStarted
                  ? [['Started', fmt(scan.started_at), false] as [string, string, boolean]]
                  : []),
                ...(filters.showCompleted
                  ? [['Completed', fmt(scan.completed_at), false] as [string, string, boolean]]
                  : []),
                ...(filters.showTrivyVersion
                  ? [
                      [
                        'Scanner Versions',
                        [
                          scan.trivy_version ? `Trivy ${scan.trivy_version}` : '',
                  ...
                  : []),
                ...(scan.helm_chart
                  ? [['Helm Chart', scan.helm_chart, true] as [string, string, boolean]]
                  : []),
                ...(scan.helm_source_path
                  ? [['Helm Source', scan.helm_source_path, false] as [string, string, boolean]]
                  : []),
                ...(scan.os_family
                  ? [
                      ['OS', `${scan.os_family} ${scan.os_name}`.trim(), false] as [
                        string,
                        string,
                        boolean,
                      ],
                  ...
                  : []),
                ...(scan.architecture
                  ? [['Architecture', scan.architecture, false] as [string, string, boolean]]
                  : []),
                ...[
                  [
                    'Provider / Source',
                    [
                      scan.scan_provider === 'artifactory_xray' ? 'Artifactory Xray' : 'Trivy',
                      scan.scan_source === 'uploaded_archive'
                        ? 'Uploaded archive'
                ...
                ...(scan.tags && scan.tags.length > 0
                  ? [
                      ['Tags', scan.tags.map((t) => t.name).join(', '), false] as [
                        string,
                        string,
                        boolean,
                      ],
              ...
              <tr>
                ...
                <td style={{ padding: '3px 8px', borderBottom: '1px solid #e5e7eb' }}>
                  <span className="print:hidden">
                    <label htmlFor={`image-location-${scan.id}`} className="sr-only">
                      Registry or image location
                    </label>
                    <input
                      id={`image-location-${scan.id}`}
                      type="text"
                      value={imageLocation}
                    ...
                    {locationError && (
                      <span
                        role="alert"
                        style={{ display: 'block', fontSize: 12, color: '#dc2626' }}
                      >
                        {locationError}
                      </span>
                    )}
      ...
      {manualLoading && (
        <p
          role="status"
          aria-live="polite"
          style={{ marginBottom: 12, fontSize: 12, color: '#6b7280' }}
        >
          Loading manual findings…
        </p>
      )}
      {manualError && (
        <div
      ...
      {filters.showPolicyDetails && (scan.compliance_summary || scan.blocked_policy_details) && (
        <div
          className="report-policy-summary"
          style={{
            marginBottom: 20,
            padding: '10px 12px',
            border: '1px solid #e5e7eb',
            borderLeft: `4px solid ${scan.compliance_summary?.status === 'fail' || scan.blocked_policy_details ? '#dc2626' : '#16a34a'}`,
            borderRadius: 4,
            background: '#f9fafb',
            fontSize: 12,
          }}
        >
          ...
          {scan.compliance_summary && (
            <p style={{ margin: '4px 0 0', color: '#374151' }}>
              Organization policies: {scan.compliance_summary.status.toUpperCase()} ·{' '}
              {scan.compliance_summary.pass_count} passed · {scan.compliance_summary.fail_count}{' '}
              failed
              {scan.compliance_summary.failed_policy_names?.length
                ? ` · Failed: ${scan.compliance_summary.failed_policy_names.join(', ')}`
                : ''}
            </p>
          )}
          {scan.blocked_policy_details && (
            <p style={{ margin: '4px 0 0', color: '#374151', overflowWrap: 'anywhere' }}>
              ...
              {scan.blocked_policy_details.blocking_policies?.length
                ? ` · Policies: ${scan.blocked_policy_details.blocking_policies.join(', ')}`
                : ''}
              {scan.blocked_policy_details.matched_watches?.length
                ? ` · Watches: ${scan.blocked_policy_details.matched_watches.map((watch) => watch.name).join(', ')}`
                : ''}
            </p>
          )}
        </div>
      ...
      >
        ...
        {customFields.map((cf, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
            ...
            <button
              type="button"
              aria-label={`Remove custom field ${cf.label}`}
        ...
        >
          ...
          <button
            ...
            style={{
              background: newCustomField.label ? '#374151' : '#e5e7eb',
              color: newCustomField.label ? '#fff' : '#9ca3af',
              border: 'none',
              borderRadius: 4,
              padding: '4px 12px',
              fontWeight: 700,
              fontSize: 12,
              cursor: newCustomField.label ? 'pointer' : 'not-allowed',
              whiteSpace: 'nowrap',
            }}
          >
            + Add
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          {manualFindings.map((f) => (
            ...
            >
              <button
                type="button"
                aria-label={`Delete manual finding ${f.vuln_id}`}
              ...
              {f.title && (
                <p style={{ margin: '0 0 3px', fontWeight: 600, fontSize: 12 }}>{f.title}</p>
              )}
              {f.pkg_name && (
                <p style={{ margin: 0, fontSize: 12, color: '#444' }}>
                  <span style={{ fontWeight: 600 }}>{f.pkg_name}</span>{' '}
                  {f.installed_version && (
                    <span style={{ color: '#dc2626' }}>{f.installed_version}</span>
                  )}
                  {f.fixed_version && (
                    <>
                      {' '}
                      → <span style={{ color: '#16a34a' }}>fix: {f.fixed_version}</span>
                    </>
                  )}
                </p>
              )}
              {f.justification && (
                <div
                  style={{
                    marginTop: 6,
                    background: 'color-mix(in srgb, var(--accent) 18%, white)',
                    border: '1px solid color-mix(in srgb, var(--accent) 62%, white)',
                ...
                >
                  ...
                  <p style={{ margin: 0, fontSize: 12, color: '#4c1d95', lineHeight: 1.5 }}>
                    {f.justification}
                  </p>
                </div>
              )}
      ...
      <div className="print:hidden" style={{ marginBottom: 20 }}>
        <button
          type="button"
          aria-expanded={showAddFinding}
          aria-controls={`manual-finding-form-${scan.id}`}
          onClick={() => setShowAddFinding((v) => !v)}
          style={{
            background: showAddFinding ? '#f3f4f6' : 'var(--accent)',
            color: showAddFinding ? '#374151' : '#fff',
            border: 'none',
            borderRadius: 6,
            padding: '6px 14px',
            fontWeight: 700,
        ...
        {showAddFinding && (
          <div
            id={`manual-finding-form-${scan.id}`}
            style={{
              marginTop: 10,
              border: '1px solid #e5e7eb',
              borderRadius: 8,
          ...
          >
            {(
              [
                ...
                ['title', 'Title'],
              ] as [keyof ManualFinding, string][]
            ).map(([field, label]) => (
              <div key={field}>
                <label
                  htmlFor={`manual-finding-${scan.id}-${field}`}
                  style={{
                    display: 'block',
                    fontSize: 12,
                    color: '#6b7280',
                ...
                <input
                  id={`manual-finding-${scan.id}-${field}`}
                  type="text"
            ...
            <div>
              <label
                htmlFor={`manual-finding-${scan.id}-severity`}
                style={{
                  display: 'block',
                  fontSize: 12,
                  color: '#6b7280',
              ...
              <select
                id={`manual-finding-${scan.id}-severity`}
            ...
            <div>
              <label
                htmlFor={`manual-finding-${scan.id}-cvss-score`}
                style={{
                  display: 'block',
                  fontSize: 12,
                  color: '#6b7280',
              ...
              <input
                id={`manual-finding-${scan.id}-cvss-score`}
                type="number"
                min={0}
                max={10}
                step={0.1}
                ...
                onChange={(e) =>
                  setNewFinding((f) => ({ ...f, cvss_score: parseFloat(e.target.value) || 0 }))
                }
                style={{
                  width: '100%',
                  border: '1px solid #d1d5db',
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              <label
                htmlFor={`manual-finding-${scan.id}-justification`}
                style={{
                  display: 'block',
                  fontSize: 12,
                  color: '#6b7280',
              ...
              >
                Justification - Why is this CVE accepted / not a risk?
              </label>
              <textarea
                id={`manual-finding-${scan.id}-justification`}
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              <button
                ...
                disabled={!newFinding.vuln_id || manualSaving}
                aria-busy={manualSaving}
                style={{
                  background: newFinding.vuln_id ? 'var(--accent)' : '#e5e7eb',
                  color: newFinding.vuln_id ? '#fff' : '#9ca3af',
                  border: 'none',
                  borderRadius: 6,
                  padding: '7px 20px',
                  fontWeight: 700,
                  fontSize: 12,
                  cursor: newFinding.vuln_id ? 'pointer' : 'not-allowed',
                }}
              >
                {manualSaving ? 'Saving…' : 'Save Finding'}
              </button>
            </div>
          </div>
        )}
      ...
      {suppressedVulns.length > 0 && (
        <div style={{ marginBottom: 20 }}>
          ...
          <div className="report-table-scroll">
            <table
              className="report-suppressed-table"
              style={{
                width: '100%',
                borderCollapse: 'collapse',
                tableLayout: 'fixed',
            ...
            >
              <thead>
                <tr style={{ background: '#f9fafb' }}>
                  {['CVE ID', 'Package', 'Severity', 'Status', 'Source', 'Justification'].map(
                    (h) => (
                      <th
                        key={h}
                        style={{
                ...
                {suppressedVulns.map((v) => {
                  ...
                  return (
                    <tr key={v.id} style={{ color: '#6b7280' }}>
                      ...
                      <td style={{ padding: '4px 8px', border: '1px solid #e5e7eb' }}>
                        {v.suppression
                          ? (statusLabel[v.suppression.status] ?? v.suppression.status)
                          : '-'}
                      </td>
                      <td style={{ padding: '4px 8px', border: '1px solid #e5e7eb' }}>
                        {v.suppression?.source ?? 'local'}
                      </td>
                      <td
                        style={{
                          padding: '4px 8px',
...
function PrintReport() {
  ...
  useEffect(() => {
    if (!authReady || requestError || !token) {
      return;
    }
    ...
    async function loadReport() {
      ...
      const scansToLoad = requestedScanIds.length
        ? await Promise.all(
            requestedScanIds.map((scanId) => fetchScan(scanId, { signal: controller.signal }))
          )
        : helmRun
          ? await fetchRunScans(helmRun, { signal: controller.signal })
          : await fetchAllChartScans(helmChart, { signal: controller.signal });

      if (!scansToLoad.length) {
        setError(
          requestedScanIds.length
            ? 'Failed to load scans.'
            : helmRun
              ? 'No scans found for this Helm run.'
              : 'No scans found for this Helm chart.'
        );
        setData([]);
        return;
      }
      ...
      const results = await Promise.all(
        scansToLoad.map(async (scan): Promise<ScanData> => {
          const [vulns, compliance] = await Promise.all([
            fetchAllVulnerabilities(scan.id, { signal: controller.signal }),
            fetchCompliance(scan.id, { signal: controller.signal }),
          ]);
  ...
  }, [authReady, helmChart, helmRun, requestError, retryNonce, scansParam, token]);
  ...
  if (error)
    return (
      <div style={{ padding: 40, color: '#dc2626', fontFamily: 'sans-serif' }} role="alert">
  ...
  const resolvedHelmChart =
    helmChart || data.find(({ scan }) => scan.helm_chart)?.scan.helm_chart || '';
  const reportTitle = resolvedHelmChart
    ? 'Helm Chart Security Report'
      ...
      <div style={{ minHeight: '100vh', width: '100%', background: '#fff', color: '#111827' }}>
        <div className="report-workspace">
          <div className="report-document">
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 12 }}>
                  ...
                  <div>
                    ...
                    <a
                      href="https://justlab.app/en/justscan"
                      style={{
                        display: 'block',
                        marginTop: 1,
                        color: '#6b7280',
              ...
              <div style={{ display: 'flex', gap: 8, flexShrink: 0, alignItems: 'center' }}>
                ...
                <span
                  style={{
                    ...
                    color: totalActive === 0 ? '#15803d' : '#dc2626',
                    fontWeight: 700,
                    fontSize: 13,
                    padding: '4px 12px',
                    borderRadius: 999,
            ...
            {/* Per-scan sections */}
            {data.map((d, i) => (
              <ScanSection key={d.scan.id} data={d} filters={filters} isFirst={i === 0} />
            ))}
            ...
            >
              ...
              <span style={{ maxWidth: '65%', textAlign: 'right', overflowWrap: 'anywhere' }}>
                {data.map((d) => `${d.scan.image_name}:${d.scan.image_tag}`).join(', ')}
              </span>
            </div>
          </div>
