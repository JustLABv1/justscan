...
func init() {
	Migrations.MustRegister(func(ctx context.Context, db *bun.DB) error {
		statements := []string{
			`CREATE INDEX IF NOT EXISTS idx_comments_scan_id ON comments (scan_id)`,
			`CREATE INDEX IF NOT EXISTS idx_org_scans_scan_id ON org_scans (scan_id)`,
			`CREATE INDEX IF NOT EXISTS idx_compliance_results_scan_id ON compliance_results (scan_id)`,
			`CREATE INDEX IF NOT EXISTS idx_compliance_history_scan_id ON compliance_history (scan_id)`,
			`CREATE INDEX IF NOT EXISTS idx_git_repository_run_images_scan_id ON git_repository_run_images (scan_id)`,
			`CREATE INDEX IF NOT EXISTS idx_vulnerability_postures_scan_id ON vulnerability_postures (scan_id)`,
			`CREATE INDEX IF NOT EXISTS idx_sbom_dependencies_from_component_id ON sbom_dependencies (from_component_id)`,
			`CREATE INDEX IF NOT EXISTS idx_sbom_dependencies_to_component_id ON sbom_dependencies (to_component_id)`,
			`CREATE INDEX IF NOT EXISTS idx_vulnerability_component_links_component_id ON vulnerability_component_links (component_id)`,
		}
