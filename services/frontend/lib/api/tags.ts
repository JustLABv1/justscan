...
export const listTags = () => {
  ...
  return req<{ data: Tag[] }>('GET', `/api/v1/tags/${qs ? `?${qs}` : ''}`).then(
    (result) => result.data ?? []
  );
};
...
export const deleteTag = (id: string) => req<{ result: string }>('DELETE', `/api/v1/tags/${id}`);

export const listTagShares = (id: string) =>
  req<{ data: ResourceShare[] }>('GET', `/api/v1/tags/${id}/shares`).then(
    (result) => result.data ?? []
  );
...
export const transferTagOwnership = (id: string, orgId: string) =>
  req<{ result: string }>('POST', `/api/v1/tags/${id}/transfer-ownership`, { org_id: orgId });
