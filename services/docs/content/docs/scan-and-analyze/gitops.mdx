...
For private OCI or HTTP dependencies, create a JustScan registry with credentials that have pull permission. Artifactory OCI access tokens require the Artifactory username and access token together.

In a managed source, **Helm dependency credentials** has two modes:

- **Automatic matching:** JustScan selects an accessible registry that matches the dependency host. When several registry entries use the same host, the closest repository-path match is preferred.
- **A selected registry:** choose a specific JustScan registry to make credential selection deterministic. It takes precedence for dependencies on that registry's host. This is useful when one Artifactory host has separate credentials or permissions for different Helm repositories.

Only registries in the deployment repository's workspace can be selected. For organization-owned repositories, share the registry with that organization first.
