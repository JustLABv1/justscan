...
Every supported setting can be overridden with a `BACKEND_` environment variable using underscore-separated names. For example, `scanner.concurrency` becomes `BACKEND_SCANNER_CONCURRENCY`. Comma-separated callback host and CIDR values are parsed as lists. Keep `jwt.secret`, `encryption.key`, database passwords, and NVD keys in secret injection rather than plain YAML.
