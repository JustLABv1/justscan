...
description: Select a supported topology and complete the deployment sequence for a self-hosted JustScan service.
---

JustScan is delivered as separate backend, frontend, and documentation services, with PostgreSQL for persistent application data. Docker Compose supplies a single-host deployment; Helm supplies Kubernetes resources and an optional PostgreSQL subchart.

<img src="/docs/architecture.svg" alt="JustScan deployment topology showing users, nginx or ingress, frontend, backend, documentation, PostgreSQL, and scanners" />

## Select a topology

| Topology | Use when | You operate |
| --- | --- | --- |
| [Docker Compose](/operate/docker-compose) | A single host is appropriate for the expected scan volume. | nginx, named Docker volumes, images, and host-level backups. |
| [Helm](/operate/helm) | Your platform standard is Kubernetes or you need managed ingress and secrets. | Kubernetes resources, storage classes, ingress, and optionally an external PostgreSQL service. |
...
5. Open the application, docs, and Swagger on the public hostname. Then complete [production readiness](/getting-started/production-readiness) and the [first governed scan](/getting-started/first-scan).
