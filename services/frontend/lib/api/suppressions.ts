...
export const upsertSuppression = (digest: string, data: Partial<Suppression> & { org_id?: string; is_global?: boolean }) =>
...
export const listSuppressionImages = (id: string) => {
  ...
  appendScope(params);
  const suffix = params.toString() ? `?${params}` : '';
  return req<{ data: SuppressionAppliedImage[]; total: number }>('GET', `/api/v1/suppressions/${id}/images${suffix}`);
};
