...
1. Deploy the selected topology from [Docker Compose](/operate/docker-compose) or [Helm](/operate/helm).
2. Confirm `/api/v1/health` is healthy through the intended route and verify `/docs` and `/swagger` resolve on the same hostname.
3. Test login, create an organization, add a test registry, and complete a small image scan.
